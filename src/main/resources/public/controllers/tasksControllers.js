...
app.controller('EditTasksController', function($scope, $location, ListTasksByIdFactory, EditTasksFactory, $route, $routeParams ){
	...
	selectedTask.$promise.then(function(result){
		...
		$scope.editTaskCtrl.name = result.name;
		$scope.editTaskCtrl.desc = result.desc;
		$scope.editTaskCtrl.user = result.user;
		$scope.editTaskCtrl.status = result.status;
...
app.controller('TasksPrioritiesListController', function($scope) {
	$scope.items = ['Not Started','In Progress','Completed'];
});
